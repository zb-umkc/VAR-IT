...
class VARTrainer(object):
    ...
    def eval_ep(self, ld_val: DataLoader):
        ...
        for batch in ld_val:
            ...
            inp_B3HW = batch["pixel_values"].to(dist.get_device(), non_blocking=True)
            ...
            with torch.no_grad():
                ...
                logits_BLV, diff_loss, _, mask_wo_prev_stages = self.var(x_BLCv_wo_first_l, label_B, lr_inp, text_hidden=None,
                                    last_layer_gt = gt_idx_Bl[-1], last_layer_gt_discrete = gt_idx_Bl[-2], lr_inp_scale = None)
            gt_BL = torch.cat((gt_BL[:, :-self.var_wo_ddp.last_level_pns], gt_BL[:, -self.var_wo_ddp.last_level_pns:][mask_wo_prev_stages].view(B,-1)), dim=1)
            L_mean += self.val_loss(logits_BLV.contiguous().data.view(-1, V), gt_BL.view(-1)) * B
            L_tail += self.val_loss(logits_BLV.contiguous().data[:, self.start_token:].reshape(-1, V), gt_BL[:, self.start_token:].reshape(-1)) * B
    ...
    def train_step(
        ...
        with self.var_opt.amp_ctx:
            ...
            loss = logits_loss + diff_loss * 2.0 #+ degradation_loss
        ...
        if it == 0 or it in metric_lg.log_iters:
            Lmean = self.val_loss(logits_BLV.contiguous().view(-1, V), gt_BL.view(-1)).item()
        ...
        if self.wandb_flag:
            if dist.is_master():
                #wandb.log({"train/loss": loss.item(), "train/logits_loss": logits_loss.item(), "train/diff_loss": diff_loss.item(), "train/degradation_loss": degradation_loss.item(), "train/grad_norm": grad_norm, "train/lr": lr, "train/wd": wd})
                wandb.log({"train/loss": loss.item(), "train/logits_loss": logits_loss.item(), "train/diff_loss": diff_loss.item(), "train/grad_norm": grad_norm, "train/lr": lr, "train/wd": wd})
